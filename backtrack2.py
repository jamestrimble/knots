...
def mini_gap_4_check(arr):
    ...
    for i in range(-len(arr), 0):
        if arr[i] == arr[i+5] != -1:
            ...
            if a!=-1 and b!=-1 and c!=-1 and d!=-1 and a!=d:    # alldiff(a, b, c, d)
                between = False
                for j in range(i+6, i+len(arr)):
                    if arr[j]==a or arr[j]==b or arr[j]==c or arr[j]==d:
                        between = not between
                    elif between and arr[j] != -1:
                        unpaired ^= (1<<arr[j])
                if unpaired:
                    return True
        elif arr[i] == arr[i+7] != -1:
            all_set = True
            for k in range(1, 7):
                if arr[i+k]==-1:
                    all_set = False
    ...
    for i in range(-len(arr), 0):
        if arr[i]==arr[i+5]:
            ...
            if a!=d and a!=f and b!=e and c!=f:    # alldiff(a, b, c, d, e, f)
                ...
                if unpaired:
                    return True
# Commented out as it seems to make the program slightly slower
#        elif arr[i]==arr[i+3]:
#            a, b, = arr[i+1], arr[i+2]
#            between = False
#            for j in range(i+4, i+len(arr)):
#                if arr[j]==a or arr[j]==b:
#                    between = not between
#                elif between:
#                    unpaired[arr[j]] = not unpaired[arr[j]]
#            if any(unpaired):
#                return True
    return False
...
def search_recursive(N, i, min_permissible_gap, arr, results):
    if i<N-3 and i%2==1 and mini_gap_4_check(arr):
        return
    ...
    if i>1 and j == first_empty+3:
        if arr[first_empty-2]==arr[first_empty+1]:
            # Can't have anything like 0.20.2
            j+=2
                ...
                if i==2 and min_permissible_gap==2:
                    ...
                    if j-first_empty==3:# and arr[j-2]!=-1 and arr[j-1]!=-1:
                        ...
                        if arr[j+1]!=a and arr[j-1]!=a:
                            j+=2
                            continue
                elif i==2 and min_permissible_gap==4 and N>9 and arr[6]!=1 and arr[8]!=1 and arr[8]!=3 and j==2*N-3:
                    # Avoid patterns like 0 1 2 . . 0 . . . . . . . . . . 1 2 . .,
                    # which are not lex min
                        break
                elif i==2 and j<N*2-1 and arr[j+1]==1 and arr[j-2]==0:
                    # Can't have 012____0?21_____
                    j+=2
